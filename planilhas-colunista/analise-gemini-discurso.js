...
// ====== CONFIG ======
const EXCEL_FILE = "./planilhas/jose-henrique-mariante.xlsx";
const SHEET_INDEX = 0;
const COL_ID = "id";
const COL_TEXTO = "texto";

const OUTPUT_FILE = "./resultados/revisao/jose-henrique-mariante-discurso.json";

// Tempo máximo que aceitaremos o Gemini "gerando" antes de considerar travou
const GENERATION_TIMEOUT_MS = 60000; // 60s (ajuste para 40000 se quiser 40s)
...
// Novo seletor baseado no ícone "stop" (parar geração) que o Gemini exibe enquanto gera
 ...
 * - envia prompt
 * - espera terminar por completed novo (timeout curto)
...
async function processItemWithRetry(page, prompt) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS_PER_ITEM; attempt++) {
    ...
    try {
      ...
      // contador de completed antes de enviar (evita completed antigo)
      ...
      // espera terminar com timeout curto (ex: 60s)
      ...
      if (attempt < MAX_ATTEMPTS_PER_ITEM) {
        ...
        // F5 / reload e re-tenta o mesmo prompt
